...
| VIII. Performance & Caching | Redis TTLs from ARCHITECTURE.md enforced: vehicle tree 7d, article detail 24h, search 1h/5m empty, autocomplete 15m/5m empty. Price and availability are always read live; pre-checkout revalidates them. Redis is bounded with LFU eviction. No N+1 queries — Prisma `include`/`select` used. | ✅ PASS |
