...
> **Superseded (number-first routing + free-text fallback):** the four-tier exact→prefix chain below has been replaced by a simpler, evidence-driven flow in `SearchService`. There is **no number-vs-text classification** — validated against InterCars (Solr), which sends the raw query to one endpoint with no `type`/`searchMode` param and lets the score rank across number and description fields at once. Since TecDoc exposes number search (`searchType: 10`) and free-text (`searchType: 99`) as two separate calls, and the two result sets are near-disjoint, we approximate this by always running the **number lane first** (`searchType: 10` / `prefix_or_suffix` over the brand-stripped query, then the raw query if it differs) and **falling back to `searchType: 99`** over the raw query only when the number lane is empty (brand kept for free-text). The FE `exact=true` toggle (confirmed as InterCars' `qExactMatch`) is a separate bucket: `searchType: 10` / `exact` over the same candidates, no free-text fallback. Worst case 3 calls (2 number + 1 free-text); a real part number resolves on the first. The zero-result "did you mean" recovery below is retained. The capability matrix and payload shapes below remain accurate. Autocomplete's own number/category dual-lane behaviour is tracked as a separate task.
...
**Future evolution path**: When traffic justifies it, implement the TecDoc Data Package + OpenSearch route and retire the two-call number/free-text split — a single index searching number and description fields together (as InterCars/Solr does) removes the fallback entirely. `searchType: 99` (free-text description search) is now wired in as the fallback lane (see the superseded note above); its response shape is verified against the live endpoint to match the number search's, top-level keys and article keys alike.
