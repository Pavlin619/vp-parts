...
- **FR-015**: Anonymous visitors MUST be able to add items to a cart. That cart is held server-side from the first add and identified by an opaque cart token the browser keeps; it MUST survive a page reload and a browser restart. When an anonymous visitor attempts to proceed to checkout, they MUST be redirected to login/registration with their cart and intended destination preserved. On successful login, their cart MUST be merged into their account cart before checkout continues. See `docs/CART.md`.
...
A cart line is identified by brand *and* article number throughout. An article number alone is not an identity — two data suppliers file the same number for different parts.

- **FR-018**: Every cart MUST be persisted server-side — a guest's against a cart token, a customer's against their account — so a customer's cart is consistent across devices and browser sessions, and so checkout reads what is being bought from a store the browser cannot rewrite. A cart row MUST hold only intent (article identity, quantity, selection) plus a snapshot of the catalogue text; it MUST NOT hold a price that is displayed. Guest carts MUST expire after 30 days of inactivity; customer carts MUST NOT expire.
- **FR-019**: On login, the guest cart MUST be merged into the account cart in a single server-side transaction: the union of both, with quantities summed for duplicate articles and a line selected if either side had it selected. The guest cart is then retired and its token released. This merge MUST happen before the customer reaches the checkout step. Where the union would exceed the cart line limit, the account's own lines are kept first and the guest's oldest next; the lines that did not fit MUST be reported to the customer rather than dropped silently.
