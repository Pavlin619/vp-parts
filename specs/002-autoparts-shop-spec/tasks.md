...
>
> **⚠️ Later update (error handling):** the single-article `InventoryService.getAvailability` method and its `AvailabilityDto` were **removed** — it duplicated `getBestPriceAndAvailability` and only differed by failing closed. Error handling is now fixed per read shape: the **single**-article read (`getBestPriceAndAvailability`) **fails open** (neutral "unavailable" — a lone buy box has nothing else to show), while the **bulk** read (`getBulkPricesAndAvailability`: listing grid, search, substitutes, and the future checkout re-validation) **fails closed** by throwing `InventoryUnavailableException` (503 / `INVENTORY_UNAVAILABLE`) so a whole list is never rendered as falsely out of stock — callers surface a single "try again later" state. Only read *failures* throw; an article with genuinely no stock still resolves to `available: false`. Where tasks below mention `getAvailability` / `AvailabilityDto`, read them against this update.
...
- [X] T059 [US3] Write integration tests for the availability endpoint (GET /inventory/articles/:articleNumber/availability — `Cache-Control: no-store`, mocked `PrismaService`) in `apps/api/test/inventory.e2e-spec.ts` — **superseded**: the standalone endpoint was removed; availability is served via `GET /catalog/articles/:n?include=availability` (covered by `catalog.e2e-spec.ts`) and `inventory.e2e-spec.ts` was deleted.
...
- [X] T064 [US3] Implement the inventory API function (`getAvailability` + `availabilityQueryOptions`, `cache: 'no-store'`) in `apps/web/src/lib/api/inventory.ts` — **superseded**: removed with the standalone endpoint; the frontend reads availability via `getArticleAvailability` (`?include=availability`) in `apps/web/src/lib/api/catalog.ts`.
...
- [ ] T113b [US5] Delivery-promise pre-action re-validation (proposal 4 from the delivery-staleness work; see `docs/DELIVERY-LOGIC.md` → "Snapshot freshness"). The detail-page delivery date is an SSR snapshot that the client only soft-refreshes (`computedAt`/`cutoffAt` staleness guard + `useDeliveryRefresh`). At the checkout confirm step, `CheckoutService` calls the live, fail-closed `InventoryService.getBulkPricesAndAvailability(cart numbers, { withWarehouses: true })` **in-process** (recomputing `availabilityByWarehouse` with a fresh `now`) and binds the **returned** delivery date as the committed promise, so a stale tab can never commit a wrong date. The bulk read fails closed (throws `InventoryUnavailableException`), which is exactly what a binding commitment needs. Surface a "delivery date changed" notice if it shifted, mirroring the existing price-change detection in `CheckoutService` (T091). The single-article buy box read (`getBestPriceAndAvailability`) fails open for display — there is no separate inventory HTTP endpoint. Wire in `apps/api/src/orders/checkout.service.ts`.
