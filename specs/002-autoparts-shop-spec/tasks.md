...
- [X] T054 [US2] Implement search API functions (searchByPartNumber, getAutocomplete) in `apps/web/src/lib/api/catalog/` (extend existing file)
...
>
> **⚠️ Later update (stock is matched on the whole article identity):** a stock line belongs to an article only when `tecdoc_number` **and** `tecdoc_supplier_id` both match — number-only matching was pricing one brand's part off another's line (13,596 numbers in `supplier_stock` are filed by more than one supplier; 23% of measured matches went to the wrong brand). The backoffice now indexes both tables on the pair and grants the shop role `SELECT` on `supplier_stock.tecdoc_supplier_id`; the repositories match on both columns in the `WHERE` clause (`findByArticle` / `findByArticles`, the batch one joining the wanted pairs in through `unnest`), so the database narrows and nothing post-filters in TypeScript. `InventoryService.getAvailability` takes `ArticleIdentityDto[]` and answers a map keyed by `articleIdentityKey(brandId, articleNumber)`, and the endpoint is `GET /catalog/articles-availability?articles=268:WL6340` — where tasks below say `?numbers=…`, grouping "by `tecdoc_number`", or availability "keyed by number", read them against this. Stock filed under an internal OE code rather than a TecDoc supplier id is deliberately unreachable — a separate relation, see `TODO(inventory-oe-parts)`.
...
- [X] T064 [US3] Implement the inventory API function (`getAvailability` + `availabilityQueryOptions`, `cache: 'no-store'`) in `apps/web/src/lib/api/inventory.ts` — **superseded**: removed with the standalone endpoint; the frontend reads availability via `getArticlesAvailability` (`GET /catalog/articles-availability`) in `apps/web/src/lib/api/catalog/availability.ts`.
